...
static int futex_wait(addr_t uaddr, dword_t val, struct timespec *timeout) {
    ...
    if (futex_load(futex, &tmp))
        ...
        for (;;) {
            ...
            // Periodic spurious wake to keep Bun/JSC parking-lot mutexes
            // from livelocking under our userspace scheduler when a wake
            // gets lost. Linux already permits spurious wakeups (libc
            // users must re-check the guarded word), so this is safe.
            // Returning 0 makes glibc/musl re-test the futex word and
            // re-park if still equal. The previous OpenMinis safety
            // valve called do_exit_group(0) here, which silently killed
            // claude-code with no output.
            if (timeout == NULL && stall_count >= 50) { // 5 s
                err = 0;
                stall_count = 0;
                break;
            }
